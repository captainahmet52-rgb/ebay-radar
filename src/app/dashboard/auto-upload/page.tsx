...
export default function AutoUploadPage() {
  ...
  const SOURCE_CURRENCY_SYMBOL: Record<string, string> = { US: "$", UK: "£", DE: "€" };
      ...
      <div style={{ display: "grid", gridTemplateColumns: "5fr 7fr", gap: "1.5rem" }}>
        ...
        <div>
          ...
          <div style={{ ...cardStyle, marginTop: "0.75rem", marginBottom: 0 }}>
            ...
            {!isCrossMarket ? (
              <Hint>
                Şu an aynı ülkeden alıp satıyorsun (Amazon {settings.uploadSourceMarket} → eBay{" "}
                {settings.uploadEbaySite.replace("EBAY_", "")}) — ekstra bir maliyet yok, bir şey
                yapmana gerek yok.
              </Hint>
            ) : (
              <>
                <div style={{ padding: "10px 12px", borderRadius: 8, background: "rgba(88,166,255,0.08)", border: "1px solid rgba(88,166,255,0.25)", marginBottom: 10 }}>
                  <div style={{ fontSize: "0.82rem", color: C.text, lineHeight: 1.5 }}>
                    <strong style={{ color: C.accent2 }}>Farklı ülkeden alıp satıyorsun</strong> (Amazon{" "}
                    {settings.uploadSourceMarket} → eBay {settings.uploadEbaySite.replace("EBAY_", "")}).
                    eBay&apos;in bu tür satışlarda kestiği gizli ücretleri sistem otomatik hesaba
                    katıyor — kâr marjın her zaman senin yazdığın gibi net kalır, ekstradan
                    hiçbir şey yapmana gerek yok.
                  </div>
                </div>
                <FieldLabel>Ek Kargo/Gümrük Payı (ürün başına, {sourceCurrencySymbol})</FieldLabel>
                <input type="number" style={inputStyle}
                  value={settings.crossExtraFixed} min={0} step={0.5}
                  onChange={(e) => set("crossExtraFixed", parseFloat(e.target.value) || 0)}
                />
                <Hint>
                  Amazon {settings.uploadSourceMarket}&apos;den alışverişte gördüğün ekstra
                  uluslararası kargo ücreti varsa buraya yaz. Bilmiyorsan 0 bırak — sorun değil,
                  sadece bu tek kalemi kaçırmış oluruz, gerisi otomatik.
                </Hint>
              </>
            )}
          </div>
