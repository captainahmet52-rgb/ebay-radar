      ...
      {(data) => {
        ...
        return (
          <div className="flex -m-6 lg:-m-8 min-h-[calc(100vh-0px)]">
            {/* İç Sidebar */}
            <aside className="w-[200px] flex-shrink-0 border-r border-[#292524] pt-5 pb-6 px-3">
              <p className="text-[10px] font-bold text-[#57534e] uppercase tracking-widest mb-3 px-2">
                Alt Kategoriler
              </p>
                  ...
                  >
                    ...
                    <span
                      className={cn(
                        ...
                        active ? "bg-[#f1641e] text-[#0c0a09]" : "bg-[#292524] text-[#78716c]"
                      )}
                    >
            ...
            <div className="flex-1 pt-5 pb-8 px-6 overflow-y-auto">
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-2 text-sm">
                  <span className="text-[#78716c]">Talep Üzerine Üretim</span>
                  <span className="text-[#44403c]">›</span>
                  <span className="font-semibold text-[#f5f5f4]">{activeCategory.name}</span>
                </div>
                <span className="text-xs text-[#78716c] bg-[#1c1917] border border-[#292524] px-3 py-1 rounded-full">
...
function ProductCard({
  ...
  return (
    <div className="bg-[#1c1917] border border-[#292524] rounded-2xl overflow-hidden flex flex-col hover:border-[#44403c] transition-colors">
      {/* Görseller */}
      <div className="relative grid grid-cols-2 h-44">
        <ImageSlot src={img1} label="Satış" />
        <ImageSlot src={img2} label="Üretim" />
        {products.length > 0 && (
          <div className="absolute bottom-0 left-0 right-0 bg-[#0c0a09]/85 backdrop-blur-sm py-1.5 px-3 flex items-center gap-1.5">
            <span className="text-[#d65212] text-xs">◈</span>
            <span className="text-[11px] text-[#a8a29e] font-medium">Örnek Ürünler</span>
      ...
      <div className="p-4 flex flex-col gap-3 flex-1">
        <div>
          <p className="text-sm font-bold text-[#fafaf9] leading-snug">{sub}</p>
        ...
        {/* Fiyat tablosu */}
        {hasSalePrice ? (
          <div className="bg-[#0c0a09] rounded-xl p-3 space-y-1.5">
          ...
          <div className="bg-[#0c0a09] rounded-xl p-3 text-xs text-[#78716c] text-center py-4">
...
function ImageSlot({ src, label }: { src: string | null; label: string }) {
  return (
    <div className="relative overflow-hidden bg-[#292524] border-r border-[#44403c] last:border-r-0">
      {src ? (
        // eslint-disable-next-line @next/next/no-img-element
        <img src={src} alt={label} className="w-full h-full object-cover" />
      ) : (
        <div className="w-full h-full flex items-center justify-center">
          <span className="text-[#44403c] text-2xl">◈</span>
        </div>
      )}
      <span
        className={cn(
          ...
          label === "Satış"
            ...
            : "bg-[#1c1917]/80 text-[#a8a29e] border border-[#44403c]/60"
        )}
      >
        {label}
      </span>
...
function PriceRow({
  ...
  return (
    <div className="flex items-center justify-between gap-2">
      <span className={cn("text-xs", bold ? "text-[#d6d3d1] font-medium" : "text-[#57534e]")}>{label}</span>
      <span
        className={cn(
          "text-xs",
          bold ? "text-[#f5f5f4] font-semibold" : muted ? "text-[#57534e]" : "text-[#78716c]",
          italic && "italic"
        )}
      >
        {value}
